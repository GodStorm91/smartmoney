...
class RewardContract(Base):
    ...
    __table_args__ = (
        Index(
            "ix_reward_contracts_user_chain_address",
            "user_id",
            "chain_id",
            "contract_address",
            unique=True,
...
class CryptoSyncState(Base):
    ...
    # Relationships
    wallet: Mapped["CryptoWallet"] = relationship(
        "CryptoWallet", back_populates="sync_states", lazy="select"
    )

    __table_args__ = (
        Index(
            "ix_crypto_sync_user_wallet_chain", "user_id", "wallet_address", "chain_id", unique=True
        ),
    )
...
class RewardClaim(Base):
    ...
    __table_args__ = (Index("ix_reward_claims_tx_chain", "tx_hash", "chain_id", unique=True),)
...
class DefiPositionSnapshot(Base):
    ...
    protocol_apy: Mapped[Decimal | None] = mapped_column(
