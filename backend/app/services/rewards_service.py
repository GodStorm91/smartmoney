...
class RewardsService:
    ...
    def create_custom_avatar(self, user_id: int, image_filename: str) -> Avatar:
        code = f"custom_{user_id}_{uuid.uuid4().hex[:8]}"
        avatar = Avatar(
            code=code,
            name="Custom Avatar",
            emoji=None,
        ...
        # Unlock and activate for user
        user_avatar = UserAvatar(user_id=user_id, avatar_id=avatar.id, is_active=True)
        self.db.add(user_avatar)
        self.db.commit()
