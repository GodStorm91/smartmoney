...
interface ReportInlineActionProps {
  action: PendingAction
}

export function ReportInlineAction({ action }: ReportInlineActionProps) {
  const { t, i18n } = useTranslation('common')
  const navigate = useNavigate()
  const { executeMutation, dismissMutation, undoMutation } = usePendingActions('report_page')
