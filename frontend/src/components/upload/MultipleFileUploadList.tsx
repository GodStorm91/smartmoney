...
export function MultipleFileUploadList({
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      <div className="flex items-center justify-between gap-4 pb-4 border-b border-gray-200">
        <div className="flex items-center gap-4 text-sm">
          ...
          {successFiles.length > 0 && (
            <div>
              <span className="font-medium text-income-600 dark:text-income-300">{successFiles.length}</span>
              <span className="ml-1 text-gray-600">{t('upload.uploaded')}</span>
            </div>
          )}
          {errorFiles.length > 0 && (
            <div>
              <span className="font-medium text-expense-600 dark:text-expense-300">{errorFiles.length}</span>
      ...
      {uploading && (
        <div className="flex items-center gap-2 text-sm text-primary-600 dark:text-primary-300 bg-primary-50 dark:bg-primary-900/20 p-3 rounded-lg">
