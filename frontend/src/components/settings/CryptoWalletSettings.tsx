...
export function CryptoWalletSettings({ variant = 'card' }: CryptoWalletSettingsProps) {
  ...
  const totalCount = (wallets?.length || 0) + (contracts?.length || 0)

  if (walletsLoading || contractsLoading) {
    if (variant === 'collapsible') {
      return (
        <CollapsibleSection
          title={t('crypto.title', 'Crypto Wallets')}
        ...
        <div className="space-y-6">
          ...
          <div>
            <div className="flex justify-between items-center mb-4">
              <h4 className="font-medium text-gray-900 dark:text-gray-100">{t('crypto.wallets')}</h4>
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
            <div className="flex justify-between items-center mb-4">
              <div>
                <h4 className="font-medium text-gray-900 dark:text-gray-100">{t('crypto.rewardContracts')}</h4>
                <p className="text-sm text-gray-500 dark:text-gray-400">{t('crypto.rewardContractsDescription')}</p>
              </div>
              <Button variant="outline" size="sm" onClick={() => setShowContractForm(!showContractForm)}>
  ...
  // Helper render functions for reuse
  function renderWalletForm() {
    if (!showWalletForm) return null
    return (
  ...
  function renderWalletList() {
    if (!wallets || wallets.length === 0) {
      return <p className="text-gray-400 dark:text-gray-500 text-center py-4">{t('crypto.noWallets')}</p>
    }
    return (
        ...
        {wallets.map((wallet: CryptoWallet) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 ml-4">
              ...
              >
                {syncingWalletId === wallet.id ? <LoadingSpinner size="sm" /> : t('crypto.sync')}
              </Button>
              <button
                onClick={() => {
  ...
  function renderContractForm() {
    if (!showContractForm) return null
    return (
      <div className="mb-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
  ...
  function renderContractList() {
    if (!contracts || contracts.length === 0) {
      return <p className="text-gray-400 dark:text-gray-500 text-center py-4">{t('crypto.noContracts')}</p>
    }
    return (
      <div className="space-y-3">
